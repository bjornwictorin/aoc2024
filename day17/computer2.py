...
def exec_instr(instr, operand, reg_a, reg_b, reg_c, pc, output):
    if instr == 0: # adv
        ...
        output.append(get_combo(operand, reg_a, reg_b, reg_c) & 7)
        pc += 2
    elif instr == 6: # bdv
        combo_op = get_combo(operand, reg_a, reg_b, reg_c)
...
def main():
    ...
    candidates = [0]
    for ii in range(16):
        output = program[15 - ii]
        new_candidates = []
        for candidate in candidates:
            for next_3_bits in range(8):
                reg_a = 8 * candidate + next_3_bits
                if calc_output(reg_a, program) == program[15 - ii:]:
                    new_candidates.append(reg_a)
        candidates = new_candidates
        # print(candidates)

    assert calc_output(min(candidates), program) == program
